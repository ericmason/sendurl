defmodule SendurlWeb.URLLive.FormComponent do
  use SendurlWeb, :live_component

  alias Sendurl.Locations

  @impl true
  def update(%{url: url} = assigns, socket) do
    changeset = Locations.change_url(url)
  ...
  defp save_url(socket, :edit, url_params) do
    case Locations.update_url(socket.assigns.url, url_params) do
      {:ok, _url} ->
        {:noreply,
         socket
  ...
  defp save_url(socket, :new, url_params) do
    case Locations.create_url(url_params) do
      {:ok, _url} ->
        {:noreply,
         socket
